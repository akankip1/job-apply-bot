...
function fallbackLocationAnswer(field, profile) {
  ...
  if ((label.includes("united states") || label.includes("us")) && (country === "united states" || state)) return "Yes";
  if (label.includes("european union") || label.includes("eu")) return "No";

  const nearbyCities = NEARBY_CITY_GROUPS[city] || [];
  if (nearbyCities.some((nearbyCity) => label.includes(nearbyCity))) return "Yes";
  if (label.includes("florida") && city === "seattle") return "No";
...
async function classifyField(field, profile, answers, schema, context = {}) {
  ...
  // Location yes/no fallback
  const locationAnswer = fallbackLocationAnswer(field, profile);
  if (locationAnswer) {
    return planned(field, "locationFact", locationAnswer, "location.fallback");
  }
