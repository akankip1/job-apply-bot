...
async function classifyField(field, profile, answers, schema, context = {}) {
  const label = normalizeText(field.label);
  const id = normalizeText(field.id);
  const log = typeof context.log === "function" ? context.log : null;
  ...
  // LLM factual reasoning fallback, gated by policy.
  try {
    const { getPolicy } = require("./answerPolicy");
    const policy = getPolicy(field, profile, answers, schema);
    if (policy.allowed) {
      const { planAnswer } = require("./llmAnswerPlanner");
      const llmResult = await planAnswer(field, profile, answers, schema, log);

      if (llmResult.manualReview) {
        return manual(field, llmResult.reason || "llm_manual_review");
      }

      if (llmResult.confidence === "high" && llmResult.safeToFill && llmResult.answer) {
        return planned(field, policy.key || "locationFact", llmResult.answer, `llm:${llmResult.source || "planner"}`, false);
      }

      return manual(field, llmResult.reason || "llm_rejected");
    }
  } catch (e) {
    if (log) {
      log("llm_answer_rejected", {
        fieldId: field.fieldId,
        label: field.label,
        reason: `llm_error:${e.message}`,
      });
    } else {
      console.warn(`LLM answer planning failed: ${e.message}`);
    }
  }
...
async function createAnswerPlan(schema, profile, answers, context = {}) {
  const decisions = [];
  for (const field of schema.fields) {
    decisions.push(await classifyField(field, profile, answers, schema, context));
  }
