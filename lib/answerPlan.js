...
const RULES = [
  ...
  { id: "cover_letter", key: "coverLetter", source: p => p.standard.coverLetterPath },
  ...
  { label: /basis of your current work authorization|current work authorization.*u s citizen|permanent resident|f 1 opt|j 1|h 1b/, key: "workAuthorizationBasis", sensitive: true, source: (p, ans) => ans.work_authorization_basis },
...
async function classifyField(field, profile, answers, schema, context = {}) {
  ...
  // Location yes/no fallback (and rule-based planner)
  const llmResult = await planAnswer(field, profile, answers, schema, log);
  if (llmResult.safeToFill) {
    return { ...planned(field, "locationFact", llmResult.answer, llmResult.source), ...llmResult };
  }
